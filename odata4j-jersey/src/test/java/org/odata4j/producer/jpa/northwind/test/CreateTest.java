...
public class CreateTest extends JPAProducerTestBase {
  ...
  public void tunneledInsertEntityToExistingEntityRelationJson() {
    ODataConsumer consumer = ODataJerseyConsumer.newBuilder(endpointUri).setFormatType(FormatType.JSON).setClientBehaviors(OClientBehaviors.methodTunneling("PUT")).build();
    insertEntityToExistingEntityRelationAndTest(consumer);
  }
  ...
  public void insertEntityToExistingEntityRelationJson() {
    ODataConsumer consumer = ODataJerseyConsumer.newBuilder(endpointUri).setFormatType(FormatType.JSON).build();
    insertEntityToExistingEntityRelationAndTest(consumer);
  }
  ...
  public void insertEntityWithInlinedEntityAtom() {
    insertEntityWithInlinedEntity(ODataJerseyConsumer.create(endpointUri));
  }

  @Test
  public void insertEntityWithInlinedEntityJson() {
    insertEntityWithInlinedEntity(ODataJerseyConsumer.newBuilder(endpointUri).setFormatType(FormatType.JSON).build());
  }

  @Test
  public void expandEntitiesWithNullReferenceAtom() {
    expandEntitiesWithNullReference(ODataJerseyConsumer.create(endpointUri));
  }

  @Test
  public void expandEntitiesWithNullReferenceJson() {
    expandEntitiesWithNullReference(ODataJerseyConsumer.newBuilder(endpointUri).setFormatType(FormatType.JSON).build());
  }
