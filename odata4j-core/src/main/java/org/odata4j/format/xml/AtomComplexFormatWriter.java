...
public class AtomComplexFormatWriter extends XmlFormatWriter implements FormatWriter<ComplexObjectResponse> {
  ...
  public void write(UriInfo uriInfo, Writer w, ComplexObjectResponse target) {
    ...
    List<OProperty<?>> properties = obj.getProperties();

    if (properties.size() != 1) {
      throw new RuntimeException("AtomComplexFormatWriter requires single root property: " + properties);
    }

    writer.startDocument();
    this.writeProperty(writer, properties.get(0), true);
    writer.endDocument();
